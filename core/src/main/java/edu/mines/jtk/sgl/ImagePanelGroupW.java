 ...
 * @version 2007.01.18
 * @ wiggle version 2020.02.14 JB West
 */
public class ImagePanelGroupW extends Group {
   ...
   */
  public ImagePanelGroupW(float[][][] f) {
   ...
   * @param f 3D array of floats.
   */
  public ImagePanelGroupW(Sampling s1, Sampling s2, Sampling s3, float[][][] f) {
   ...
   */
  public ImagePanelGroupW(Sampling s1, Sampling s2, Sampling s3, Float3 f) {
    this(s1,s2,s3,f,new Axis[]{Axis.X,Axis.Y});
  }
   ...
   */
  public ImagePanelW getImagePanel(Axis axis) {
    for (ImagePanelW ip:_ipList) {
   ...
   * @return the iterator.
   */
  public Iterator<ImagePanelW> getImagePanels() {
  ...
  private ArrayList<ImagePanelW> _ipList;
    ...
    Sampling s1, Sampling s2, Sampling s3, Float3 f3, Axis[] axes) 
  {
    ...
    int np = axes.length;
    _ipList = new ArrayList<ImagePanelW>(np);
    for (int jp=0; jp<np; ++jp) {
      AxisAlignedQuad aaq = new AxisAlignedQuad(axes[jp],qmin,qmax);
      ImagePanelW ip = new ImagePanelW(s1,s2,s3,f3);
  ...
  static class WiresW extends Node {
    WiresW(ArrayList<ImagePanelW> ipList) {
      _ipListW = ipList;
    }
    protected void draw(DrawContext dc) {
      glColor3f(1.0f,1.0f,1.0f);
      glLineWidth(1.5f);
      for (ImagePanelW ip:_ipListW) {
        AxisAlignedFrame fi = ip.getFrame();
        Axis ai = fi.getAxis();
        for (ImagePanelW jp:_ipListW) {
    ...
    private ArrayList<ImagePanelW> _ipListW;
  }

  // Decorates this group with wires that show panel intersections.
  static class Wires extends Node {
    Wires(ArrayList<ImagePanelW> ipList) {
      _ipList = ipList;
    }
    protected void draw(DrawContext dc) {
      glColor3f(1.0f,1.0f,1.0f);
