 ...
 * @version 2008.08.24
 */
public class ImagePanelGroup2W extends Group {
   ...
   */
  public ImagePanelGroup2W(float[][][] f1, float[][][] f2) {
   ...
   * @return the image panel; null, if none has the axis specified.
   */
  public ImagePanelW getImagePanel2(Axis axis) {
    for (ImagePanelW ip:_ip2List) {
      if (axis==ip.getFrame().getAxis())
        return ip;
    }
   ...
   * @return the iterator.
   */
  public Iterator<ImagePanelW> getImagePanels2() {
  ...
  private ArrayList<ImagePanelW> _ip2List;
    ...
    Sampling s1, Sampling s2, Sampling s3, Float3 f1, Float3 f2, Axis[] axes) 
  {
    ...
    _ip2List = new ArrayList<ImagePanelW>(np);
    ...
    for (int jp=0; jp<np; ++jp) {
      ...
      if (axes[jp] != Axis.Z) {
        ImagePanelW ip2 = new ImagePanelW(s1,s2,s3,f2);
        ip2.skip(_skip);
        ip2.gain(_gain);
        ip2.posfill(_posfill);
        ip2.posfill(_negfill);
